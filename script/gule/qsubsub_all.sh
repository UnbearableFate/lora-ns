...
for init_lora_weights in "${init_lora_weights_list[@]}"; do
    qsub -v TRAIN_CONFIG="${TRAIN_CONFIG}",init_lora_weights="${init_lora_weights}" \
     "${run_script}"
done

for init_lora_weights in "${fit_init_lora_weights_list[@]}"; do
    qsub -v TRAIN_CONFIG="${TRAIN_CONFIG}",init_lora_weights="${init_lora_weights}",use_sr_trainer=1 \
     "${run_script}"
done